...
export default function Analysis() {
  ...
  return (
    <div>
      ...
      <CopilotSection sectionName="选择基金" contextData={{ selectedFunds: selectedFunds.map((fid) => funds.find((f) => f.id === fid)?.name) }}>
      ...
      <Row gutter={16}>
        {selectedFunds.map((fid) => {
          ...
          return <Col span={selectedFunds.length === 1 ? 24 : 12} key={fid}>
            <CopilotSection sectionName={`基金分析: ${fund?.name || ''}`} contextData={{ fundId: fid, dataPoints: (priceData[fid] || []).length }}>
