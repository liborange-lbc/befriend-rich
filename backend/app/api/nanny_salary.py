...
def add_holiday(body: HolidayIn, db: Session = Depends(get_db)):
...
def update_holiday(holiday_id: int, body: HolidayIn, db: Session = Depends(get_db)):
...
@router.post("/leaves")
def add_leave(body: LeaveIn, db: Session = Depends(get_db)):
...
def calculate_pay_dates(count: int = 5, db: Session = Depends(get_db)):
